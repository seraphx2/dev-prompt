...
| **Linux**         | Same codebase, compiles. Hotkey works on X11; Wayland needs the XDG global-shortcuts portal (tray-click fallback otherwise). Terminal-command actions need per-emulator working-dir flags (in progress) — plain "open a terminal" works. Panel renders opaque (no blur). Packaging (`deb`/`rpm`/`AppImage`) not yet in CI. |
